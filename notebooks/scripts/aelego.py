...
def get_encoder(pretrained: bool = False, frozen: bool = False):
    encoder = CNNEncoder(channels=64, depth=4, residual=True, attn=True).to(DEVICE)
    if pretrained:
...
def get_decoder():
    return CNNDecoderNoBridge(LOCAL, 1, 3, residual=True, attn=True).to(DEVICE)
